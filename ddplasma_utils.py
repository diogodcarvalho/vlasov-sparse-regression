...
import numba
import pickle
from sklearn.linear_model import ridge_regression, orthogonal_mp
...
def rebin_errors_vs_numnonzeroterms(regression_results, train_test = 'mse_test', if_display_modelformvariance = False, ax=plt):
    ...
    if if_display_modelformvariance == True:
        ...
        if train_test == 'mse_test':
            ax.errorbar(numnonzeroterms_array[idx_zero_modelformvar], ave_error[idx_zero_modelformvar],
            ...
            ax.scatter(numnonzeroterms_array[idx_zero_modelformvar], ave_error[idx_zero_modelformvar], c = 'C1', marker = 'o')
            ax.scatter(numnonzeroterms_array[~idx_zero_modelformvar], ave_error[~idx_zero_modelformvar], c = 'C1', marker = '^')
        ...
        if train_test == 'mse_test':
            ax.errorbar(numnonzeroterms_array, ave_error,
                         yerr= [min_error, max_error] , label='both limits (default)', fmt='o', c = 'C0')
        else:
            ax.scatter(numnonzeroterms_array, ave_error, c = 'C1', marker = 'o')
...
def STRidge(X, y, alpha=0, threshold=0.1, max_iter=100):
    """Sequentially thresholded Ridge as in PDE-FIND (aka SINDy for PDEs)"""
    m, d = X.shape
    ...
    for i in range(max_iter):
        ...
        small_coefs = (np.abs(xi) < threshold)
        big_coefs = np.logical_not(small_coefs)

        # check for convergence
        n_nonzero = np.sum(big_coefs)
        if n_nonzero == n_nonzero_last:
            break
        else:
            n_nonzero_last = n_nonzero
            if i == max_iter - 1:
                print('warning: STLS did not converge. increase max_iter.')

        # zero small coefs
        xi[small_coefs] = 0
        # update non-zero coefs with new ls
        if alpha == 0:
            xi[big_coefs] = np.linalg.lstsq(X[:, big_coefs],y, rcond=None)[0]
        else:
            xi[big_coefs] = ridge_regression(X[:, big_coefs],y, alpha)

    return xi


def STRidge_1by1(X, y, alpha=0, xi = None):
    """Modified Sequentially thresholded Ridge, threshold is updated based on smallest coefficient
    Equals STLS_1by1 if alpha=0"""
    m, d = X.shape
    ...
    threshold = np.min(np.abs(xi[np.nonzero(xi)]))
...
def Kfold_Regression__1by1(Theta, Y, K_folds = 10, normalize_Theta = True, algorithm='stridge', algorithm_params=dict()):
    if normalize_Theta:
...
#        n_iters = n_max_terms
    ...
    for train_index, test_index in kf.split(Theta):
        ...
        for i in range(n_iters):
            ...
            if algorithm == 'stridge':
                coefs, lamdas[i] = STRidge_1by1(X = Theta_train/Theta_Norm,
                                                y = Y_train[:,0],
                                                xi = coefs,
                                                **algorithm_params)
            if algorithm == 'omp':
                if i == 0:
                    coefs = np.zeros(n_terms)
                else:
                    coefs = orthogonal_mp(X = Theta_train/Theta_Norm,
                                          y = Y_train[:,0],
                                          n_nonzero_coefs = i)


            coefficients_evol[k, i,:] = coefs/Theta_Norm

            mse_train[k, i] = np.mean((Y_train[:,0] - np.dot(Theta_train/Theta_Norm, coefs))**2)
            mse_test[k, i] = np.mean((Y_test[:,0] - np.dot(Theta_test/Theta_Norm, coefs))**2)
...
def plot_pareto_front_analysis(regression_results, pareto_pos = None, ax=plt):
...
def discovered_Mterm_models(regression_results, description, M = 1):
    ...
    plt.yticks(range(sum(idx_non_zero_coefs_Mterm_models)), np.array(description_2)[idx_non_zero_coefs_Mterm_models], rotation='vertical')
